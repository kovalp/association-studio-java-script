...
describe('constructor', () => {
    it('defines the attributes', () => {
        ...
        front.probe.box.set_state([1.0, 2.0, 3.0, 4.0, 5.67])
        expect(front.panels.inp_wdt.value).toBeCloseTo(5.67)

        change_and_dispatch(front.panels.inp_x, 1.22333)
        expect(front.probe.box.xy_yaw_lw).toBeDeepCloseTo([1.22333, 2, 3, 4, 5.67])

        change_and_dispatch(front.panels.inp_y, 2.34)
        expect(front.probe.box.xy_yaw_lw).toBeDeepCloseTo([1.22333, 2.34, 3, 4, 5.67])

        change_and_dispatch(front.panels.inp_yaw, 90)
        expect(front.probe.box.xy_yaw_lw).toBeDeepCloseTo([1.22333, 2.34, 1.57, 4, 5.67])

        change_and_dispatch(front.panels.inp_len, 4.56)
        expect(front.probe.box.xy_yaw_lw).toBeDeepCloseTo([1.22333, 2.34, 1.57, 4.56, 5.67])

        change_and_dispatch(front.panels.inp_wdt, 2.56)
        expect(front.probe.box.xy_yaw_lw).toBeDeepCloseTo([1.22333, 2.34, 1.57, 4.56, 2.56])
        ...
        expect(front.probe.box.transform.m41).toBeCloseTo(400)
        expect(front.probe.box.transform.m42).toBeCloseTo(300)
    })
})
